...
angular.module('apparatusApp')
  .controller('RegistrationCtrl', function ($scope, $timeout, $cookies, $location, Auth, CONFIG) {
    $scope.CONFIG = CONFIG;
    ...
    $scope.loginMe = function(email, password) {
      Auth.sally.login('password', { email: email, password: password });
    };
    ...
    $scope.registerMe = function(email, password) {
      Auth.sally.createUser(email, password, function(error, user) {
        $timeout(function() {
          if (error) {
            $scope.$broadcast('auth:fail', {error: error});
          } else {
